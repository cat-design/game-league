...
function draw() {
  ...
  if (gameState === 'play') {
    ...
    if (keyDown('space')) {
      runner.velocityY = -5;
      runner.changeAnimation('jumping',redjumpingAnimation)
    }

    if (keyDown("b")) {
      boomerang1 = createSprite(runner.x, runner.y, 10, 10);
      boomerang1.addAnimation("swing", boomerang); boomerang1.velocityX = 5;
      boomerang1.scale = 0.02
      if (cactusGroup.isTouching(boomerang1)) { score = score + 20; }
      if (fireGroup.isTouching(boomerang1)) { score = score + 40; }
    }
    if (coinGroup.isTouching(runner)) {
      score = score + 10
      coinGroup.destroyEach()
    }
    if (score > 50) runner.changeAnimation('run', redrunningAnimation);
    else runner.changeAnimation('running', purplerunningAnimation);
    runner.velocityY = runner.velocityY + 0.5;
    runner.collide(invisibleGround);
    spawnFireBall();
    spawnCactus();
    spawncoin();
    if (cactusGroup.isTouching(runner) || fireGroup.isTouching(runner)) {
      gameState = 'end'
    }
  }
  if (gameState == 'end') {
...
function spawnCactus() {
  var obstacle = Math.round(random(1, 2))
...
function spawncoin() {
  if (frameCount % 80 === 0) {
    var coin1 = createSprite(600, 250, 40, 10);
    coin1.y = random(120, 200); coin1.addImage(coin);
    coin1.scale = 0.09;
    coin1.velocityX = -5;
    coin1.lifetime = 300;
    runner.depth = coin1.depth + 1;
    coinGroup.add(coin1);
  }
}